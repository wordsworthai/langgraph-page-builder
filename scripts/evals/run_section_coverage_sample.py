"""Run section-coverage eval sample on real data and store results in Mongo.
...
poetry run python -m wwai_agent_orchestration.evals.cli \
  ...
  --db-name eval \
  resume \
  --eval-set-id="section_coverage_sample_20260309_145548" \
  --max-concurrency 1
...
def main(argv):
    del argv  # Unused.
    eval_set_id = FLAGS.eval_set_id or f"section_coverage_sample_{timestamp_suffix()}"
    ...
    if not business_ids:
        raise RuntimeError("No businesses found with tag=eval for section coverage sample run.")
    else:
        print("Found", len(business_ids), "businesses for section coverage sample run.")

    # Use index 0 for all cases
    palette_data = get_color_palette_and_font(index=7)
    fixed_visual_inputs = {
    ...
    eval_set = build_eval_set(
        ...
        middle_section_count=6,
        section_query_filter={"status": "ACTIVE", "tag": "smb"},
