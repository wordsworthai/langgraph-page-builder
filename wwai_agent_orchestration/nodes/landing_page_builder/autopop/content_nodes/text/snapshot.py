# nodes/smb/autopop/content_nodes/text/snapshot.py
"""
Content text snapshot node.

Creates snapshot after all content text sections are collected.
"""
...
@NodeRegistry.register(
    name="content_text_snapshot",
    description="Create snapshot after all content text sections are collected",
    max_retries=1,
    timeout=90,
    tags=["autopopulation", "snapshot", "content", "text"],
    display_name="Finalizing text content",
    show_node=True,
    show_output=False,
)
async def content_text_snapshot(state: LandingPageWorkflowState, config: RunnableConfig = None) -> Dict[str, Any]:
    """Create snapshot after all content text sections are collected."""
    ...
    delta.setdefault("logs", []).append({"level": "info", "msg": "content_text_snapshot: snapshot created"})
    delta.setdefault("meta", {}).update({"last_node": "content_text_snapshot"})
    result = autopop_helpers.update_autopop_state(state, delta)
    result["ui_execution_log"] = [
        make_ui_execution_log_entry_from_registry("content_text_snapshot", content_text_finalized_html())
    ]
    return result
