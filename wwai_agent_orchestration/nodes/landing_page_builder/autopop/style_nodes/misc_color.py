# nodes/smb/autopop/style_nodes/misc_color.py
"""
Misc color autopopulation node.

Contains:
- misc_color_agent: Autopopulates miscellaneous element colors based on background colors
"""
...
from template_json_builder.ipsum_lorem_agents.default_content_provider.background_conditioned_misc_color_recommendation import (
    autopopulate_misc_variants_for_backgrounds,
)
from template_json_builder.models.autopop_dataclasses.prompt_spec.prompt_dataclasses import (
    BackgroundConditionedMiscElementAutopopSectionInputModel,
    BackgroundConditionedMiscElementOutputModel,
)
...
from wwai_agent_orchestration.prompt_builder.prompt_classes.landing_page_builder.autopop import misc_color_autopop
from wwai_agent_orchestration.utils.llm.model_utils import get_model_config_from_configurable


def _execute_misc_color_prompt(
    inp: BackgroundConditionedMiscElementAutopopSectionInputModel,
    bypass_prompt_cache: bool = False,
    model_config: Optional[Dict[str, Any]] = None,
) -> BackgroundConditionedMiscElementOutputModel:
    spec = misc_color_autopop.MiscColorAutoPopSpec()
    ...
    return BackgroundConditionedMiscElementOutputModel(**response.result)


def misc_color_agent_module(
    ...
    """Autopopulate miscellaneous element colors based on background colors.
    ...
    """
    module_name = AutopopulationModuleTypes.ELEMENT_MISC_COLOR.value
    ...
    if use_mock:
        ...
        output = autopopulate_misc_variants_for_backgrounds(inp)
    else:
        output = _execute_misc_color_prompt(
...
@NodeRegistry.register(
    name="misc_color_agent",
    description="Autopopulate miscellaneous element colors based on background colors using LLM",
    max_retries=1,
    timeout=150,
    tags=["autopopulation", "style", "color", "misc"],
    display_name="Choosing element colors",
    show_node=False,
    show_output=False,
)
async def misc_color_agent(state: LandingPageWorkflowState, config: RunnableConfig = None) -> Dict[str, Any]:
    """Process misc color autopopulation (runs in parallel with text and button color agents)."""
    ...
    delta = {
        ...
        "logs": [{"level": "info", "msg": "misc_color_agent: completed"}],
    }
