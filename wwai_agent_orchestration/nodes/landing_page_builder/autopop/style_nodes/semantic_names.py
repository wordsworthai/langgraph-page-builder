# nodes/smb/autopop/style_nodes/semantic_names.py
"""
Semantic names autopopulation nodes.

Contains:
- semantic_names_agent: Autopopulates semantic names for elements
- semantic_names_snapshot: Creates snapshot after semantic names autopopulation
"""
...
from template_json_builder.ipsum_lorem_agents.agent_utils import semantic_name_agent_utils
from template_json_builder.ipsum_lorem_agents.default_content_provider.semantic_name_inference import (
    infer_semantic_names,
)
from template_json_builder.models.autopop_dataclasses.prompt_spec.prompt_dataclasses import (
    SectionElementSemanticNameInferenceInputModel,
    SectionElementSemanticNameInferenceOutputModel,
)
...
from wwai_agent_orchestration.prompt_builder.prompt_classes.landing_page_builder.autopop import semantic_name_autopop
...
def _execute_semantic_name_prompt(
    inp: SectionElementSemanticNameInferenceInputModel,
    bypass_prompt_cache: bool = False,
    model_config: Optional[Dict[str, Any]] = None,
) -> SectionElementSemanticNameInferenceOutputModel:
    spec = semantic_name_autopop.SemanticNameMappingSpec()
    ...
    return SectionElementSemanticNameInferenceOutputModel(**response.result)


def semantic_name_agent_module(
    imm: AutopopulationImmutableState,
    use_mock: bool = False,
    bypass_prompt_cache: bool = False,
    model_config: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """Autopopulate semantic names for elements.
    ...
    """
    module_name = AutopopulationModuleTypes.SEMANTIC_NAME.value
    inp = semantic_name_agent_utils.build_semantic_name_agent_input(imm=imm)
    
    if use_mock:
        output = infer_semantic_names(inp)
    else:
        output = _execute_semantic_name_prompt(
...
@NodeRegistry.register(
    name="semantic_names_agent",
    description="Autopopulate semantic names for design elements using LLM",
    max_retries=1,
    timeout=150,
    tags=["autopopulation", "style", "semantic"],
    display_name="Finalizing text and button styles",
    show_node=True,
    show_output=False,
)
async def semantic_names_agent(state: LandingPageWorkflowState, config: RunnableConfig = None) -> Dict[str, Any]:
    """Autopopulate semantic names for elements."""
    ...
    res = semantic_name_agent_module(
        imm=imm,
    ...
    delta = {
        ...
        "logs": [{"level": "info", "msg": "semantic_names_agent: completed"}],
        "meta": {"last_node": "semantic_names_agent"},
    }
    result = autopop_helpers.update_autopop_state(state, delta)
    result["ui_execution_log"] = [
        make_ui_execution_log_entry_from_registry("semantic_names_agent", semantic_names_html())
    ]
    return result


@NodeRegistry.register(
    name="semantic_names_snapshot",
    description="Create snapshot after semantic names autopopulation",
    max_retries=1,
    timeout=90,
    tags=["autopopulation", "snapshot", "style"],
    display_name="Saving semantic names",
    show_node=False,
    show_output=False,
)
async def semantic_names_snapshot(state: LandingPageWorkflowState, config: RunnableConfig = None) -> Dict[str, Any]:
    """Create snapshot after semantic names autopopulation."""
    autopop_state = autopop_helpers.get_autopop_state(state)
    ...
    delta.setdefault("logs", []).append({"level": "info", "msg": "semantic_names_snapshot: snapshot created"})
    delta.setdefault("meta", {}).update({"last_node": "semantic_names_snapshot"})
